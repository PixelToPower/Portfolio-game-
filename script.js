document.addEventListener('DOMContentLoaded', () => {         //HINT : HTML Document 
    ...
    document.addEventListener('mousemove', (e) => {
        cursor.style.left = `${e.clientX - 10}px`; cursor.style.top = `${e.clientY -
            10}px`;         // HINT : Add Units for the offsets 
    });
    ...
    for (let i = 0; i < 30; i++) {                 //HINT : Range should go from 0 till 30
        particles.push({
            x: Math.random() * canvas.width, y: Math.random() *
                canvas.height, radius: Math.random() * 2 + 0.5, vx: (Math.random() - 0.5) * 0.5, vy:
                (Math.random() - 0.5) * 0.5
        });
    }
    ...
    function fireConfetti() {     //HINT : define fireConfetti function 
        ...
        for (let i = 0; i < 150; i++) {
            confettis.push({
                ...
                size: Math.random() * 10 + 5, color: colors[Math.floor(Math.random() *
                    colors.length)],
    ...
    function animateConfetti() {             //HINT : define animateConfetti function 
        ...
        confettis.forEach((c, index) => {
            ...
            cCtx.fillStyle = c.color; cCtx.fillRect(-c.size / 2, -c.size / 2, c.size,
                c.size);
            cCtx.restore();
    ...
    const gameData = {
        ...
        3: { answer: "12", question: "Evaluate the JavaScriptclosure:<br><br><code>const add = x => y => x + y;</code><br><br>What is the output of<code>add(10)(2)</code> ?" }
    ...
    document.addEventListener('mousemove', (e) => {       //Hint : Event where mouse moves
        ...
        room.style.transform = `translateZ(200px) rotateX(${-pctY * 15}deg)
                    rotateY(${pctX * 30}deg)`;
    });
    ...
    function checkPuzzle() {                //HINT : Define checkPuzzle function 
        ...
        if (guess === gameData[activeDoorId].answer) {
            // Success! 
            const currentDoorFrame =
                document.querySelector(`.door-frame[data-id="${activeDoorId}"]`);
            ...
            if (activeDoorId < 3) {        //HINT : if activeDoorId is less than 3 
                ...
                //HINT : Text that appears on the door when it is UNLOCKED 
    ...
    document.querySelectorAll('.close-btn, .cancel-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                if (finalDoor.classList.contains('solved') &&
